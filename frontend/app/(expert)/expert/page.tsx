...
type ContentItem = { id: string; views?: number };
...
function sumViews(items?: ContentItem[]) {
  return Array.isArray(items) ? items.reduce((sum, item) => sum + (item.views || 0), 0) : 0;
}
...
/**
 * Reads the display name straight out of the JWT already stored in
 * localStorage (payload shape: sub, email, role, name, iat, exp) —
 * no extra profile endpoint required.
 */
...
export default function ExpertOverviewPage() {
  ...
  async function loadDashboard() {
    try {
      ...
      if (!token) {
        setError("Vous devez être connectée pour voir votre tableau de bord.");
        return;
      }
      ...
      setError(
        err instanceof Error ? err.message : "Impossible de charger votre tableau de bord."
      );
    } finally {
      setLoading(false);
    }
  ...
  const totals = useMemo(() => {
    ...
    const views = courses.reduce((sum, c) => sum + sumViews(c.articles) + sumViews(c.videos), 0);
    ...
    <main className="min-h-screen bg-sand-50">
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="relative mb-10">
          ...
          <p className="font-script text-2xl leading-none text-rose-500">Bonjour,</p>
          <h1 className="mt-2 font-display text-3xl font-semibold text-wine-900 sm:text-4xl">
            <span className="text-gradient-rise">{name || "bienvenue"}</span> 
          </h1>
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <StatCard
            ...
            delay={0}
          />
          <StatCard icon={<Layers size={20} />} label="Contenu total" value={totals.content} delay={40} />
          <StatCard
            icon={<Users size={20} />}
          ...
          <StatCard
            ...
            delay={120}
          />
          <StatCard icon={<Eye size={20} />} label="Vues totales" value={totals.views} delay={160} />
          <StatCard
            icon={<Star size={20} />}
            label="Note moyenne"
        ...
        <div className="grid gap-6 lg:grid-cols-3">
            ...
            {upcomingMeetings.length === 0 ? (
              <div className="flex h-48 flex-col items-center justify-center rounded-2xl border border-dashed border-sand-200 bg-sand-50 px-6 text-center">
                ...
                <p className="max-w-xs text-sm text-ink-soft">
                  Aucune réunion à venir. Planifiez-en une pour retrouver vos entrepreneures.
                </p>
              </div>
            ) : (
              <ul className="divide-y divide-sand-100">
                {upcomingMeetings.slice(0, 4).map((meeting) => (
                  <li key={meeting.id} className="flex items-center gap-4 py-4">
          ...
          <section className="card-surface p-6 shadow-card">
            <h2 className="font-display text-lg font-semibold text-wine-900">Aperçu du contenu</h2>
            <p className="mt-1 text-xs text-ink-soft">Ce que vous avez publié jusqu'ici</p>

            <div className="mt-5 space-y-3">
              <ContentRow icon={<Newspaper size={16} />} label="Articles" value={totals.articles} />
              <ContentRow icon={<Film size={16} />} label="Vidéos" value={totals.videos} />
              <ContentRow icon={<FolderOpen size={16} />} label="Ressources" value={totals.resources} />
            </div>
    ...
    >
      <div
        className={`mb-3 flex h-11 w-11 items-center justify-center rounded-xl ${
          tone === "gold" ? "bg-gold-400/15 text-gold-500" : "bg-rise-gradient-soft text-wine-700"
        }`}
      >
        {icon}
      </div>
      ...
      <p className="mt-1 font-display text-2xl font-semibold text-wine-900">{value}</p>
    </div>
  );
}
...
function ContentRow({ icon, label, value }: { icon: ReactNode; label: string; value: number }) {
  return (
    <div className="flex items-center justify-between rounded-xl bg-sand-50 px-4 py-3">
      ...
      <span className="font-display text-base font-semibold text-wine-900">{value}</span>
    </div>
  );
}