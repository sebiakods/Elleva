...
const META_POLL_MS = 15000; // unread counts + thread status
...
type User = {
  ...
  bio?: string | null;
  verified?: boolean; // institutions: whether verification is already complete
};
...
type Message = {
  ...
  internal?: boolean; // true for admin-to-admin coordination notes
};
...
// Roles the logged-in ADMIN can reach out to / be reached by.
const categories = [
  {
    key: "ENTREPRENEUR",
    title: "Entrepreneurs",
    description: "Support, account issues and complaints.",
    icon: Users,
  },
  {
    key: "EXPERT",
    title: "Experts",
    description: "Account management, content and platform updates.",
    icon: GraduationCap,
  },
  {
    key: "INSTITUTION",
    title: "Institutions",
    description: "Verification, financing programs and partnerships.",
    icon: Building2,
  },
  {
    key: "ADMIN",
    title: "Admin Team",
    description: "Internal coordination between admins.",
    icon: ShieldCheck,
  },
] as const;
...
const TOPICS_BY_CATEGORY: Record<CategoryKey, { key: string; label: string }[]> = {
  ...
  INSTITUTION: [
    ...
    { key: "FINANCING", label: "Financing program" },
...
const PRIORITIES: { key: NonNullable<Message["priority"]>; label: string; dot: string }[] = [
  { key: "LOW", label: "Low", dot: "bg-slate-400" },
  { key: "NORMAL", label: "Normal", dot: "bg-sky-500" },
  { key: "HIGH", label: "High", dot: "bg-amber-500" },
  { key: "URGENT", label: "Urgent", dot: "bg-red-600" },
];

const QUICK_ACTIONS: Record<CategoryKey, { icon: any; text: string }[]> = {
  ENTREPRENEUR: [
    { icon: FileWarning, text: "Could you share more details about the issue you're experiencing?" },
    { icon: UserCog, text: "Your account has been reviewed — here's an update:" },
    { icon: CheckCircle2, text: "We've received your complaint and are looking into it. Thanks for your patience." },
  ],
  EXPERT: [
    { icon: UserCog, text: "Can you confirm a few account details so we can complete this update?" },
    { icon: Newspaper, text: "Your content submission needs a revision — here's why:" },
    { icon: Rocket, text: "Heads up — we're rolling out a platform update that affects your workflow:" },
  ],
  INSTITUTION: [
    { icon: BadgeCheck, text: "Your verification documents are being reviewed — we'll follow up shortly." },
    { icon: Landmark, text: "We'd like to discuss your financing program in more detail." },
    { icon: Handshake, text: "Proposing a partnership opportunity — are you available for a call?" },
  ],
  ADMIN: [
    { icon: ArrowUpRight, text: "Escalating this ticket to you — please review when you can." },
    { icon: UserCog, text: "Can you take over this conversation? Context below." },
    { icon: CircleDot, text: "Quick sync on today's reports/tickets?" },
  ],
};
...
export default function AdminMessagesPage() {
  ...
  const [activePriority, setActivePriority] = useState<Message["priority"]>(null);
  ...
  // Unread counts + per-thread ticket status (open/resolved), polled together.
  const loadMeta = useCallback(async () => {
    try {
      const [unreadRes, statusRes] = await Promise.all([
        fetch(`${API_URL}/messages/unread-counts`, { headers: { "user-id": currentUserId } }),
        fetch(`${API_URL}/messages/thread-status`, { headers: { "user-id": currentUserId } }),
      ]);
      if (unreadRes.ok) {
      ...
      if (statusRes.ok) {
        ...
        if (data && typeof data === "object") setThreadStatus(data);
      }
    } catch (error) {
  ...
  async function setThreadStatusFor(otherUserId: string, status: ThreadStatus) {
    setStatusUpdating(true);
    setThreadStatus((prev) => ({ ...prev, [otherUserId]: status }));
    try {
      const res = await fetch(`${API_URL}/messages/${otherUserId}/status`, {
        method: "PATCH",
        headers: {
      ...
      console.error("SET THREAD STATUS ERROR:", error);
      // revert on failure
      loadMeta();
    } finally {
  ...
  async function sendMessage(otherUserId: string) {
    ...
    const isInternal = selectedRole === "ADMIN";
    ...
    try {
      ...
      // Sending a fresh admin reply implicitly reopens a resolved ticket.
      if (threadStatus[otherUserId] === "RESOLVED") {
        setThreadStatusFor(otherUserId, "OPEN");
      }
    } catch (err) {
  ...
  // "Other Admins" is optional — hide the tab once we know there's no one there.
  const visibleCategories = useMemo(() => {
    if (loading) return categories;
    return categories.filter((c) => c.key !== "ADMIN" || (counts.ADMIN ?? 0) > 0);
  }, [loading, counts]);

  const openTicketCounts = useMemo(() => {
    return categories.reduce<Record<string, number>>((acc, c) => {
      acc[c.key] = users.filter(
        (u) => u.role === c.key && threadStatus[u.id] === "OPEN"
      ).length;
      return acc;
    }, {});
  }, [users, threadStatus]);
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((user) => {
      ...
      if (user.id === currentUserId) return false; // never message yourself in "Other Admins"
      if (!q) return true;
      return (
  ...
  const activeUserThreadStatus = activeId ? threadStatus[activeId] ?? "OPEN" : "OPEN";
    ...
    <div className="min-h-screen bg-sand-50">
      <div className="mx-auto max-w-5xl px-6 py-8 md:px-8">
        ...
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-2xl text-wine-700 md:text-3xl">Admin Inbox</h1>
            <p className="text-sm text-ink-soft">
              Handle support, verification and platform requests across the community.
            </p>
          </div>
        </div>
        ...
        <div
          ...
          className={`mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 ${
            visibleCategories.length >= 4 ? "lg:grid-cols-4" : "lg:grid-cols-3"
          }`}
        >
          {visibleCategories.map((category) => {
            ...
            const openTickets = openTicketCounts[category.key] ?? 0;
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {openTickets > 0 && (
                    <p className={`mt-1 text-[10px] font-medium ${isActive ? "text-white/90" : "text-amber-600"}`}>
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            filteredUsers.map((user) => {
              ...
              const status = threadStatus[user.id] ?? "OPEN";
                  ...
                  <div className="flex w-full items-center gap-4 p-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {selectedRole === "INSTITUTION" && user.verified && (
                          <span className="inline-flex items-center gap-0.5 rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-medium text-emerald-600">
                    ...
                    <div
                      className={`border-t px-4 py-3 ${
                        selectedRole === "ADMIN"
                          ...
                          : "border-rose-100 bg-sand-50/60"
                      }`}
                    >
                      {selectedRole === "ADMIN" && (
                        <div className="mb-2 flex items-center gap-1.5 rounded-lg bg-amber-100 px-2.5 py-1.5 text-[11px] font-medium text-amber-700">
                          <ShieldCheck size={12} /> Internal thread — visible to admins only.
                        </div>
                      )}

                      {/* Ticket status + topic + priority controls */}
                        ...
                        {selectedRole !== "ADMIN" && (
                          <button
                            ...
                            onClick={() =>
                              setThreadStatusFor(
                                user.id,
                                activeUserThreadStatus === "OPEN" ? "RESOLVED" : "OPEN"
                              )
                            }
                            className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold transition disabled:opacity-60 ${
                              activeUserThreadStatus === "OPEN"
                          ...
                          >
                            {activeUserThreadStatus === "OPEN" ? (
                              <>
                                <CircleDot size={12} /> Mark resolved
                              </>
                            ) : (
                              <>
                                <CheckCircle2 size={12} /> Reopen ticket
                              </>
                            )}
                          </button>
                        )}
                      ...
                      <div className="mb-2 flex flex-wrap items-center gap-1.5">
                        <span className="text-[11px] font-medium text-ink-soft">Priority:</span>
                        {PRIORITIES.map((p) => (
                          <button
                            key={p.key}
                            onClick={() => setActivePriority((prev) => (prev === p.key ? null : p.key))}
                          ...
                          {loadingMessages ? (
                            ...
                            messages.map((m, idx) => {
                              ...
                              const priorityMeta = m.priority
                                ? PRIORITIES.find((p) => p.key === m.priority)
                                : null;
                                          ...
                                          {priorityMeta && (
                                            <span
                                              className={`inline-flex items-center gap-1 text-[10px] font-semibold ${
                                                isMine ? "text-white/80" : "text-ink-soft"
                                              }`}
                                            >
