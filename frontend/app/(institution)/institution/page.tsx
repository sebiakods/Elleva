...
type AppStatus = "SUBMITTED" | "UNDER_REVIEW" | "APPROVED" | "REJECTED" | "WAITLISTED";
...
interface OverviewData {
  ...
  statusDistribution: { label: string; value: number; tone: BadgeTone }[];
  recentApplications: {
    id: string;
...
const statusMap: Record<AppStatus, { label: string; tone: BadgeTone }> = {
  SUBMITTED: { label: "Reçue", tone: "wine" },
...
async function fetchInstitutionOverview(): Promise<OverviewData> {
  ...
  const res = await fetch(`${API_BASE_URL}/institution/overview`, {
    headers: {
  ...
  if (!res.ok) {
    ...
    throw new Error(body?.message ?? "Impossible de charger le tableau de bord");
  }
...
function EmptyState({ icon: Icon, message }: { icon: LucideIcon; message: string }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-sand-100 bg-sand-100/40 py-10 text-center">
      ...
      <p className="max-w-[220px] text-sm text-ink-soft">{message}</p>
    </div>
  );
}
...
export default function InstitutionOverviewPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setData(await fetchInstitutionOverview());
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="mb-8 text-sm text-ink-soft">
        ...
        <span className="font-medium text-wine-700">Aperçu</span>
      </div>
      ...
      <div className="relative mb-10">
        ...
        <h1 className="mt-2 font-display text-3xl font-semibold text-wine-900 sm:text-4xl">
          Tableau de bord - <span className="text-gradient-rise">Aperçu</span>
        </h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-ink-soft">
          Consultez l&apos;activité globale de votre institution, suivez l&apos;évolution
          des candidatures et gardez un œil sur vos programmes actifs et bénéficiaires.
        </p>
      </div>
            ...
            items={[
              {
                ...
                value: stats.activePrograms.toLocaleString("fr-FR"),
                icon: Landmark,
                tone: "rose",
              },
              {
                label: "Candidatures reçues",
                value: stats.totalApplications.toLocaleString("fr-FR"),
                icon: ClipboardList,
                tone: "wine",
              },
              {
                label: "Événements à venir",
                value: stats.upcomingEvents.toLocaleString("fr-FR"),
                icon: CalendarDays,
                tone: "gold",
              },
              {
                label: "Bénéficiaires totaux",
                value: stats.totalBeneficiaries.toLocaleString("fr-FR"),
                icon: Users,
                tone: "rose",
              },
            ]}
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <Reveal delay={80} className="card-surface p-6 shadow-card">
          <div className="mb-5 flex items-center justify-between">
            ...
            <Button href="/institution/applications" variant="ghost" size="sm">
              Tout voir
            </Button>
          </div>
          ...
          {isInitialLoading ? (
            ...
            <EmptyState icon={Inbox} message="Aucune candidature reçue pour le moment." />
          ) : (
            <ul className="divide-y divide-sand-100">
              {recentApplications.map((a) => {
                const s = statusMap[a.status] ?? { label: a.status, tone: "neutral" as const };
                return (
                  <li key={a.id} className="flex items-center gap-3 py-3">
                    <Avatar name={a.applicantName} src={a.applicantAvatarUrl ?? undefined} size="sm" />
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-ink">{a.applicantName}</p>
                      <p className="truncate text-xs text-ink-soft">{a.programTitle}</p>
                    </div>
                    <div className="flex shrink-0 items-center gap-2">
                      <Badge tone={s.tone}>{s.label}</Badge>
                      <span className="hidden items-center gap-1 text-xs text-ink-soft sm:inline-flex">
                        <Clock size={11} />
                        {formatRelativeFr(a.createdAt)}
                      </span>
                    </div>
                  </li>
                );
        ...
        <Reveal delay={120} className="card-surface p-6 shadow-card">
          ...
          {isInitialLoading ? (
            ...
            <EmptyState icon={ClipboardList} message="Aucune candidature à répartir pour le moment." />
          ) : (
            <div className="space-y-4">
          ...
          <div className="mt-5 flex gap-3">
            <Button href="/institution/applications" variant="secondary" size="sm" className="flex-1">
              Voir les candidatures
            </Button>
            <Button href="/institution/analytics" variant="outline" size="sm" className="flex-1">
