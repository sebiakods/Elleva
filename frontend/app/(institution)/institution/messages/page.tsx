...
type User = {
  ...
  bio?: string | null;
  stage?: string | null; // entrepreneur growth stage, if provided
};
...
// Roles the logged-in INSTITUTION can reach out to / be reached by.
const categories = [
  {
    key: "ENTREPRENEUR",
    title: "Entrepreneurs",
    description: "Funding programs, applications, partnerships and opportunities.",
    icon: Users,
  },
  {
    key: "ADMIN",
    title: "Support Team",
    description: "Account verification, program management and platform support.",
    icon: ShieldCheck,
  },
  {
    key: "EXPERT",
    title: "Experts",
    description: "Collaborate on mentoring, events and business initiatives.",
    icon: GraduationCap,
  },
] as const;
...
const TOPICS_BY_CATEGORY: Record<CategoryKey, { key: string; label: string }[]> = {
  ...
  ADMIN: [
    ...
    { key: "PROGRAM_MANAGEMENT", label: "Program management" },
    { key: "PLATFORM_SUPPORT", label: "Platform support" },
  ],
  EXPERT: [
    { key: "MENTORING", label: "Mentoring" },
    { key: "EVENT", label: "Event" },
    { key: "INITIATIVE", label: "Business initiative" },
  ],
};

const APPLICATION_STATUSES: { key: ApplicationStatus; label: string; icon: any; className: string }[] = [
  { key: "PENDING", label: "Pending", icon: Clock3, className: "bg-slate-100 text-slate-600" },
  { key: "UNDER_REVIEW", label: "Under review", icon: ClipboardCheck, className: "bg-amber-100 text-amber-700" },
  { key: "APPROVED", label: "Approved", icon: CircleCheck, className: "bg-emerald-100 text-emerald-700" },
  { key: "REJECTED", label: "Rejected", icon: CircleX, className: "bg-red-100 text-red-700" },
];

const QUICK_ACTIONS: Record<CategoryKey, { icon: any; text: string }[]> = {
  ENTREPRENEUR: [
    { icon: Landmark, text: "We're opening applications for a new funding program — interested?" },
    { icon: FileText, text: "Could you send over any missing documents for your application?" },
    { icon: Handshake, text: "We'd like to explore a partnership opportunity with you." },
    { icon: Sparkles, text: "There's an opportunity that matches your profile — want details?" },
  ],
  ADMIN: [
    { icon: BadgeCheck, text: "Requesting verification for our institution profile." },
    { icon: Settings2, text: "Need help managing one of our active programs." },
    { icon: Headset, text: "We're experiencing a platform issue — can you assist?" },
  ],
  EXPERT: [
    { icon: Lightbulb, text: "Would you be open to mentoring founders in our upcoming cohort?" },
    { icon: CalendarPlus, text: "We're hosting an event and would love your involvement." },
    { icon: Rocket, text: "Proposing a joint business initiative — are you available for a call?" },
  ],
};
...
export default function InstitutionMessagesPage() {
  ...
  // Unread counts + per-entrepreneur application status, polled together.
  const loadMeta = useCallback(async () => {
    try {
      const [unreadRes, statusRes] = await Promise.all([
        fetch(`${API_URL}/messages/unread-counts`, { headers: { "user-id": currentUserId } }),
        fetch(`${API_URL}/messages/application-status`, { headers: { "user-id": currentUserId } }),
      ]);
      if (unreadRes.ok) {
      ...
      if (statusRes.ok) {
        ...
        if (data && typeof data === "object") setApplicationStatus(data);
      }
    } catch (error) {
  ...
  async function setApplicationStatusFor(otherUserId: string, status: ApplicationStatus) {
    setStatusUpdating(true);
    setApplicationStatus((prev) => ({ ...prev, [otherUserId]: status }));
    try {
      const res = await fetch(`${API_URL}/messages/${otherUserId}/application-status`, {
        method: "PATCH",
        headers: {
  ...
  // "Expert" collaboration tab is optional — only show it once experts exist.
  const visibleCategories = useMemo(() => {
    if (loading) return categories;
    return categories.filter((c) => c.key !== "EXPERT" || (counts.EXPERT ?? 0) > 0);
  }, [loading, counts]);

  const pendingApplicationCounts = useMemo(() => {
    return users.filter(
      (u) =>
        u.role === "ENTREPRENEUR" &&
        (applicationStatus[u.id] === "PENDING" || applicationStatus[u.id] === "UNDER_REVIEW")
    ).length;
  }, [users, applicationStatus]);
  ...
  const activeApplicationStatus = activeId ? applicationStatus[activeId] ?? "PENDING" : "PENDING";
    ...
    <div className="min-h-screen bg-sand-50">
      <div className="mx-auto max-w-5xl px-6 py-8 md:px-8">
        ...
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-2xl text-wine-700 md:text-3xl">Institution Inbox</h1>
            <p className="text-sm text-ink-soft">
              Manage funding, partnerships and support in one place.
            </p>
          </div>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {category.key === "ENTREPRENEUR" && pendingApplicationCounts > 0 && (
                    <p className={`mt-1 text-[10px] font-medium ${isActive ? "text-white/90" : "text-amber-600"}`}>
                      {pendingApplicationCounts} application{pendingApplicationCounts === 1 ? "" : "s"} pending
                    </p>
                  )}
                </div>
              </button>
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            filteredUsers.map((user) => {
              ...
              const appStatus =
                selectedRole === "ENTREPRENEUR" ? applicationStatus[user.id] : undefined;
              const appStatusMeta = appStatus
                ? APPLICATION_STATUSES.find((s) => s.key === appStatus)
                : null;
                ...
                >
                  ...
                  <div className="flex w-full items-center gap-4 p-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {appStatusMeta && (
                          <span
                            className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${appStatusMeta.className}`}
                          >
                            <appStatusMeta.icon size={10} />
                            {appStatusMeta.label}
                          </span>
                        )}
                      </div>
                  ...
                  {isOpen && (
                    <div className="border-t border-rose-100 bg-sand-50/60 px-4 py-3">
                      {/* Topic selector + application status (entrepreneurs only) */}
                      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                        ...
                        </div># retained for layout symmetry
                      </div>

                      {selectedRole === "ENTREPRENEUR" && (
                        <div className="mb-2 flex flex-wrap items-center gap-1.5">
                          <span className="text-[11px] font-medium text-ink-soft">Application:</span>
                          {APPLICATION_STATUSES.map((s) => (
                            <button
                              key={s.key}
                              disabled={statusUpdating}
                              onClick={() => setApplicationStatusFor(user.id, s.key)}
                              className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-medium transition disabled:opacity-60 ${
                                activeApplicationStatus === s.key
                                  ? s.className
                                  : "bg-white text-ink-soft border border-rose-100 hover:bg-rose-50"
                              }`}
                            >
                              <s.icon size={11} />
                              {s.label}
