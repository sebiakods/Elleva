...
async function fetchInstitutionAnalytics(
  ...
  const res = await fetch(
    `${API_BASE_URL}/institution/analytics?period=${encodeURIComponent(
      period
    )}`,
    {
      method: "GET",
  ...
  if (!data || typeof data !== "object") {
    throw new Error(
      "Les données analytiques reçues sont invalides."
    );
  }
...
export default function InstitutionAnalyticsPage() {
  ...
  const fundedCount =
    data?.statusBreakdown?.find(
      (status) => status.name === "Financées"
    )?.value ?? 0;

  const hasAmountsData =
    (data?.amountsByProgram?.length ?? 0) > 0;

  const hasStatusData =
    (data?.statusBreakdown ?? []).reduce(
      (sum, status) => sum + status.value,
      0
    ) > 0;

  const hasSectorData =
    (data?.sectorBreakdown?.length ?? 0) > 0;

  const hasApplicationsData =
    (data?.applicationsByMonth?.length ?? 0) > 0;
    ...
    <div className="mx-auto max-w-7xl space-y-10">
      ...
      <div className="relative mb-10">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-ink-soft">
          Suivez la performance de vos programmes,
          les candidatures, les financements et
          l&apos;impact de votre institution.
        </p>
      </div>
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-sm leading-6 text-ink-soft">
              Suivez la performance de vos programmes
              de financement : candidatures reçues,
              taux de conversion, montants engagés
              et impact sur l&apos;écosystème
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <Card hover={false}>
              ...
              {!hasApplicationsData ? (
                <ChartEmptyState
                  icon={BarChart2}
                  message="Aucune candidature enregistrée sur cette période."
                />
              ) : (
            ...
            <Card hover={false}>
              ...
              {hasAmountsData ? (
                ...
                <ChartEmptyState
                  icon={Landmark}
                  message="Aucun montant engagé sur cette période."
                />
              )}
            </Card>
