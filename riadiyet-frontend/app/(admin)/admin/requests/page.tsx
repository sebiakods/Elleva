  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        const [expertResponse, institutionResponse] =
          await Promise.all([
            fetch(`${API_URL}/api/expert-applications`, {
              headers: token
                ? {
                    Authorization: `Bearer ${token}`,
                  }
            ...
            fetch(`${API_URL}/api/institution-applications`, {
              headers: token
                ? {
                    Authorization: `Bearer ${token}`,
                  }
        ...
        const expertData =
          await expertResponse.json();

        const institutionData =
          await institutionResponse.json();

        const experts = (
          expertData.applications || []
        ).map((item: any) => ({
          ...item,
          type: "EXPERT",
        }));
